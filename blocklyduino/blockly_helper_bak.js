...
var code = "";
var BT_ARR = new Array();	// 명령어 집합.
var BT_ARR_IDX = 0;			// 명령어 집합 인덱스.
...
async function BT_Data_Sending() {
	...
	for(key in OneLine_Arr)
	{
		if(OneLine_Arr[key] != "")
			length++;
			// alert("OneLine_Arr[key] = " + OneLine_Arr[key]);
	}
	...
	for(var i = 0; i <= length; i++)
	{
		...
		{
			var Comments = OneLine_Arr[i].split("//");
			...
			switch(Action[0])
			{
				...
				case " FORSTART":
					...
					var Repeat = Action[2] - Action[1] + 1;
					var line = 0;
				...
				case " REPEATSTART":
					...
					{
						// alert("OneLine_Arr[i+line] = " + OneLine_Arr[i+line]);
						while(OneLine_Arr[i+line] != "  } // REPEATEND")
						{
							// alert("OneLine_Arr[i+line] = " + OneLine_Arr[i+line]);
							line++;
							if(OneLine_Arr[i+line].indexOf("//") != -1)
							{
								...
								if(operator[1] != " REPEATEND")
								{
									// alert("operator = " + operator[1]);
									// alert("operator = " + operator);
									
									var div_oper = operator[1].split("_");
									
									if(div_oper[0] == " IFSTART")
									{
										// alert("operator = " + operator);
										if_line = i+line+1;
										...
										if(div_oper[1] == "true")
										{
											while(true)
											{
												var if_oper = OneLine_Arr[if_line].split("//");
												if(if_oper[1] == " IFEND")
													break;
												
												var if_true_arr = OneLine_Arr[if_line].split("//");
												var if_true = if_true_arr[1].split("_");
												
												BT_ARR[BT_ARR_IDX] = if_true;
												BT_ARR_IDX++;
												if_line++;
											}
										}
									}
									else if(div_oper[0] == " ELSEIFSTART")
									{
										// alert("operator = " + operator);
										if_line = i+line+1;
				...
				case " NOTONE":
					...
					break;
				case " LED":
					// alert("switch in LED // i = " + i);
					BT_ARR[BT_ARR_IDX] = Action;
					BT_ARR_IDX++;
					break;
				case " DELAY":
					// alert("switch in DELAY");
					BT_ARR[BT_ARR_IDX] = Action;
					BT_ARR_IDX++;
					break;
				case " MOTOR":
			...
			switch(Action[0])
			{
				case " FORSTART":
					...
					// Action[2] = end index;
					// Action[3] = plus or minus;
					// var start_index = 0;
					// var end_index = 0;
					// var operator = Action[3] == "++" ? "++" : "--";
	...
	{
		// alert("BT_ARR[" + k + "][0] = " + BT_ARR[k][0]);
		// alert("BT_ARR[" + k + "] = " + BT_ARR[k]);
		// BT_Sending_Work(BT_ARR[k]);
		...
		switch(BT_ARR[k][0])
		{
			...
			case " TONE":
				...
				uploadCode(code, "http://127.0.0.1:8000/tone_state.html?state=play&fre=" + fre, "get", function (status, errorInfo) {});
				break;
			case " NOTONE":
...
async function BT_Sending_Work(Action)
{
	// alert("Action = " + Action);
...
/*
async function BT_Data_Sending() {
	// var code = Blockly.mainWorkspace.getAllBlocks();
	// var code = Blockly.mainWorkspace.getBlockById();	-> return null;
	var code = Blockly.mainWorkspace.getAllBlocks_BT_Data();
	...
	{
		// alert("code[i] = " + code[i]);
		var Instruction = code[i] + '';	// string으로 인식을 못하기 때문에 ''을 + 해준다.
		
		// alert("Instruction = " + Instruction);
		
		var Inst_Arr = Instruction.split(" ");
		
		// for(var j in Inst_Arr)
			// alert("Inst_Arr[" + j + "] = " + Inst_Arr[j]);
		
		
		switch(Inst_Arr[0])
		{
			case "DigitalWrite":
				// alert("DigitalWrite PIN#");
				var state = "";
				if(Inst_Arr[4] == "HIGH")
				{
					state = "on";
				}
				else if(Inst_Arr[4] == "LOW")
				{
					state = "off"
				}
			...
			case "Delay":
				// alert("Delay");
				var delay_time = Inst_Arr[1];
				await sleep(delay_time);
				break;
			case "set":
				if(Inst_Arr[1] == "motor" && Inst_Arr[2] == "direction")
				{
					var direction = "";
					if(Inst_Arr[3] == "FORWARD")
					{
						direction = "forward";
					}
					if(Inst_Arr[3] == "BACKWARD")
					{
						direction = "backward";
					}
					else if(Inst_Arr[3] == "LEFT")
					{
						direction = "left";
					}
					else if(Inst_Arr[3] == "RIGHT")
					{
						direction = "right";
					}
					else if(Inst_Arr[3] == "STOP")
					{
						direction = "stop";
					}
					...
					uploadCode(code, "http://127.0.0.1:8000/motor_direction.html?direction=" + direction, "get", function (status, errorInfo) {});
				}
			...
			case "Tone":
				...
				if(Inst_Arr[1] == "play")
				{
					// frequency = Inst_Arr[4];
					frequency = "tone";
				}
				else if(Inst_Arr[1] == "stop")
				{
					var frequency = "notone";
				}
				
				uploadCode(code, "http://127.0.0.1:8000/tone.html?frequency=" + frequency, "get", function (status, errorInfo) {});
				break;
			default:
				// alert("trash~");
				break;
